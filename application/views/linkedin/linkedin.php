<link href="<?php echo base_url('/assets/css/style1.css')?>" rel="stylesheet">
<body class="back">
  <div class="header">
    <div class="wil">
      <a href="<?php echo base_url('index.php/login/login/linkedin');?>" class="wil1">
      What is LinkedIn?
      </a>
    </div>
    <div class="jt">
      <a href="<?php echo base_url('index.php/login/login/join');?>" class="jt1">
      Join Today
      </a>
    </div>
    <div class="si">
      <a href="<?php echo base_url('index.php/login/login/signin');?>" class="si1">
      Sign In
      </a>
    </div>
      <div class="logo">
      <a href="<?php echo base_url('index.php/login/login/index');?>" class="logo1">
      <img src="<?php echo base_url('/assets/img/logo.png');?>">
      </a>
    </div>
  </div>

<div class="back2">
  <div class="mainheading">
    The world's largest professional network:

        <div class="mainheading1">225 million strong.</div>
  </div>
    <div class="abc">
    
    <div class="box1">Learn and share...
      <img src="<?php echo base_url('/assets/img/page3a.png');?>" width="250px" height="250px">
      <div class="norm-text">Get the latest news, inspiration, and insights you need to be great at what you do.</div>

    </div>
    
    <div class="box">Power your career.
      <img src="<?php echo base_url('/assets/img/page3.png');?>" width="250px" height="250px">
      <div class="norm-text">Discover professional opportunities, business deals, and new ventures.</div>
    <div class= "free">It's free<div class="button">Join now</div></div>
    </div>
      
    <div class="box">Connect. Find. Be found.
        <img src="<?php echo base_url('/assets/img/page3b.png');?>" width="250px" height="250px">
        <div class="norm-text">Build your professional identity online and stay in touch with colleagues and classmates.</div>

    </div>

    </div>
  </div>

<div img src="<?php echo base_url('/assets/img/page3c.png');?>" width="250px" height="250px"> 
